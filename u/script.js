...
if(nuser != null){
    store.collection("users").where("handle", "==", nuser).get().then((querySnaphot) => {
        querySnaphot.forEach((doc) => {
            ...
            document.getElementById("profile").innerHTML += `<img src="${pro.pfp}" style="position: relative; top: 10px; left: 12px; width: 64px; height: 64px;"><b style="width: 50%; cursor: default; position: relative; top: -36px; left: 40px;">${pro.username}</b><p style="width: 50%; color: gray; font-size: 10px; position: relative; top: -48px; left: 101px;">${pro.handle}</p>`
        })
    });
        ...
        querySnaphot.forEach((doc) => {
            ...
            <div style="border: 3px solid; position: realative; right: 0;">
                ...
                <div style="position: relative;">
                    <pre style="width: 86.6%; cursor: default; position: relative; left: 10px; white-space: pre-wrap;">${nob.content}</pre>
                </div>
